...
async function findDuplicateBookMatches(Book, inputBook, options = {}) {
  ...
  if (!isBlank(inputBook.isbn13)) {
    await addMatches({ $or: [{ isbn13: inputBook.isbn13 }, { isbn: inputBook.isbn13 }] }, 'isbn13');
  }

  if (!isBlank(inputBook.isbn10)) {
    await addMatches({ $or: [{ isbn10: inputBook.isbn10 }, { isbn: inputBook.isbn10 }] }, 'isbn10');
  }

  if (!isBlank(inputBook.isbn)) {
    await addMatches({ $or: [{ isbn: inputBook.isbn }, { isbn13: inputBook.isbn }, { isbn10: inputBook.isbn }] }, 'isbn13');
  }
